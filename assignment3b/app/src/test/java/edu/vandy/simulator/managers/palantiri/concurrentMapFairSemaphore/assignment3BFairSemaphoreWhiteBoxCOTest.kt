...
class assignment3BFairSemaphoreWhiteBoxCOTest : AssignmentTests(0) {
    @Mock
...
//    @Test
//    fun testTryToGetPermitWhenExceptionThrown() {
//        val lock = mockk<Lock>()
//        val co = spyk(FairSemaphoreCO(2), recordPrivateCalls = true)
//        lock.injectInto(co)
//
//        every { lock.lock() } returns Unit
//        every { co.tryToGetPermitUnlocked() } throws InterruptedException()
//
...
//            co.tryToGetPermit()
//            lock.lock()
//            co.tryToGetPermitUnlocked()
//            lock.unlock()
//        }
//
//        confirmVerified(lock, co)
//    }
    ...
    fun `'waitForPermit 1`() {
        val lock = mockk<Lock>()
        val waiterLock = mockk<Lock>()
        val queue = mockk<LinkedList<FairSemaphoreCO.Waiter>>()
        val waiter = mockk<FairSemaphoreCO.Waiter>()
        ...
        queue.injectInto(co)
        2.injectInto(co)
        waiterLock.injectInto(waiter)
        condition.injectInto(waiter)

        every { co.createWaiter() } returns waiter
        every { queue.add(any()) } returns true
        every { lock.unlock() } returns Unit
        every { waiterLock.lock() } returns Unit
        every { waiterLock.unlock() } returns Unit
        var count = 2
        every { condition.await() } answers { waiter.mReleased = --count == 0 }
        ...
        confirmVerified(co, waiterLock, queue, lock, condition)
    }
    ...
    fun `'waitForPermit 2`() {
        val lock = mockk<Lock>()
        val waiterLock = mockk<Lock>()
        val queue = spyk<LinkedList<FairSemaphoreCO.Waiter>>()
        val waiter = mockk<FairSemaphoreCO.Waiter>()
        val condition = mockk<Condition>()
        waiter.mReleased = false
        ...
        every { co.createWaiter() } returns waiter
        every { queue.add(any()) } returns true
        every { queue.remove(any()) } returns true
        every { lock.unlock() } returns Unit
        every { lock.lock() } returns Unit
        every { waiterLock.lock() } returns Unit
        every { waiterLock.unlock() } returns Unit
        every { condition.await() } throws InterruptedException()
    ...
    fun `'waitForPermit 3`() {
        ...
        every { co.createWaiter() } returns waiter
        every { co.release() } returns Unit
        every { queue.add(any()) } returns true
        every { queue.remove(any()) } returns false
        every { lock.unlock() } returns Unit
        every { lock.lock() } returns Unit
        every { waiterLock.lock() } returns Unit
        every { waiterLock.unlock() } returns Unit
