...
class Assignment2BReentrantLockHashMapSimpleSemaphoreMgrTest : AssignmentTests(10) {
    companion object {
    ...
    @Test(timeout = 1000)
    fun `acquire attempts to acquire and lock a palantir at the correct time`() {
        ...
        confirmVerified(semaphore, lock, manager, palantirMap, palantirList, lock)
    }

    @Test(timeout = 1000)
    fun `acquire should always unlock the reentrant lock`() {
        mockkStatic(AssignmentTests::class)
        every { lock.lockInterruptibly() } answers {
    ...
    @Test(timeout = 1000)
    fun `release calls unlock at the appropriate location`() {
        every { lock.lockInterruptibly() } throws AssignmentTests.SimulatedException()
        every { palantirMap.replace(palantir, true) } throws SimulatedException()

        val palantir = Palantir(manager)

        // SUT
        assertFailsWith<AssignmentTests.SimulatedException> { manager.release(palantir) }
